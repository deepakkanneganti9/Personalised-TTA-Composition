...
from tta_techniques.tent_adapter import set_seed  # noqa: E402
...
def load_clean_cifar_test(data_root: Path, sample_count: int, dataset_name: str) -> Subset:
...
def load_composition_model(weights_path: Path, dataset_name: str):
...
def compute_quality_factor(batch_accuracies: Sequence[float]) -> float:
    ...
    # CIFAR-100 clean-test composition accuracy is materially lower than the
    # original MNIST-tuned floor used in the copied pipeline.
    return float(np.clip(stretch_score(base_quality, floor=0.10), 0.0, 100.0))
...
def compute_reliability_metrics(
    ...
    return {
        ...
        "reliability_score": float(np.clip(stretch_score(base_reliability, floor=0.10), 0.0, 100.0)),
    }
...
def build_row(
    ...
    dataset_name = summary.get("dataset_name", "cifar100")
    num_classes = int(get_dataset_spec(dataset_name)["num_classes"])

    model = load_composition_model(composition_dir / "composition_model.pt", dataset_name=dataset_name)
    predictions, labels, raw_latency = evaluate_predictions(model, dataset, batch_size=batch_size)

    prediction_distribution = compute_prediction_distribution(predictions, num_classes=num_classes)
...
def main() -> None:
    ...
    parser.add_argument(
        "--summary-source",
        default=str(PROJECT_ROOT / "composability-results-chunked" / "summary.csv"),
    )
    ...
    dataset = load_clean_cifar_test(data_root=data_root, sample_count=args.sample_count, dataset_name="cifar100")
