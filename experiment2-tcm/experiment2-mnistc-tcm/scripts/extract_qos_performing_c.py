...
from tta_techniques.tent_grad_adapter import MNISTCNN, set_seed  # noqa: E402
...
def load_clean_mnist_test(data_root: Path, sample_count: int) -> Subset:
...
def load_composition_model(weights_path: Path) -> MNISTCNN:
...
def compute_quality_factor(batch_accuracies: Sequence[float]) -> float:
    ...
    return float(np.clip(stretch_score(base_quality, floor=0.85), 0.0, 100.0))
...
def compute_reliability_metrics(
    ...
    return {
        ...
        "reliability_score": float(np.clip(stretch_score(base_reliability, floor=0.82), 0.0, 100.0)),
    }
...
def main() -> None:
    ...
    dataset = load_clean_mnist_test(data_root=data_root, sample_count=args.sample_count)
