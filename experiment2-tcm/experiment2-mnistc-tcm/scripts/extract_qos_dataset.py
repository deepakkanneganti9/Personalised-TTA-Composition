...
from tta_techniques.tent_grad_adapter import LocalMNISTC, MNISTCNN, set_seed  # noqa: E402
...
def build_common_sample_count(artifact_dirs: Sequence[Path], mnist_c_root: Path) -> int:
    counts = []
    for artifact_dir in artifact_dirs:
        config = read_json(artifact_dir / "config.json")
        corruption = config["corruption"]
        split = config.get("split", "test")
        dataset = LocalMNISTC(root=mnist_c_root, corruption=corruption, split=split)
        counts.append(len(dataset))
    return int(min(counts))


def load_adapted_model(weights_path: Path) -> MNISTCNN:
...
def main() -> None:
    ...
    parser.add_argument("--mnist-c-root", default=str(PROJECT_ROOT / "data" / "mnist_c"))
    ...
    common_sample_count = build_common_sample_count(artifact_dirs, mnist_c_root)
