...
FL_RUN_DIR = PROJECT_ROOT / "FL" / "artifacts" / "cifar10_fl_baseline_5clients_run_clean"
TTA_DIR = PROJECT_ROOT / "TTA techniques"
if str(TTA_DIR) not in sys.path:
    sys.path.insert(0, str(TTA_DIR))

from tta_techniques.tent_adapter import LocalCIFAR10C  # noqa: E402
...
def main():
    ...
    parser.add_argument("--cifar10-c-root", type=Path, default=PROJECT_ROOT / "Data" / "CIFAR-10-C")
    ...
    with raw_rows_path.open("w", newline="", encoding="utf-8") as raw_handle:
        ...
        for adapted_case, adapted_info in adapted_static.items():
            ...
            if corruption not in cached_datasets:
                cached_datasets[corruption] = LocalCIFAR10C(
                    root=args.cifar10_c_root.resolve(),
                    corruption=corruption,
                    split="test",
