...
from tta_techniques.tent_adapter import BNStatisticsRecorder, evaluate_prediction_distribution, save_json
from FL.train_fedavg_cifar10 import CIFAR10CNN


DEFAULT_FL_RUN_DIR = PROJECT_ROOT / "FL" / "artifacts" / "cifar10_fl_baseline_5clients_run_clean"
...
def generate_one(composition_name: str, client_names: List[str], output_root: Path, fl_run_dir: Path):
    ...
    composition_model = build_model_from_state(composition_state)
    composition_weights = {
...
def main():
    ...
    for composition_name, client_names in compositions.items():
        ...
        print(f"{composition_name} | clean_cifar10_test", flush=True)
        index_rows.append(
            {
                "composition_name": composition_name,
                "client_names": client_names,
