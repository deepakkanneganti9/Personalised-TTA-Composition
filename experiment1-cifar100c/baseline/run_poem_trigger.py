...
class PoemConfig:
    ...
    cifar_output_threshold_sweep_csv: str
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--cifar_clean_window_csv",
        default="outputs_step1_step2_cifar_30k_5r_2e_fixed/historical_bank/clean_validation_window_summary.csv",
    )
    parser.add_argument(
        "--cifar_step5_window_csv",
        default="outputs_step1_step2_cifar_30k_5r_2e_fixed/pools/step5_window_scan.csv",
    )
    parser.add_argument("--cifar_output_per_window_csv", default="poem/outputs/cifar_poem_per_window.csv")
    parser.add_argument("--cifar_output_per_corruption_csv", default="poem/outputs/cifar_poem_per_corruption.csv")
    parser.add_argument("--cifar_output_per_corruption_json", default="poem/outputs/cifar_poem_per_corruption.json")
    parser.add_argument("--cifar_output_threshold_sweep_csv", default="poem/outputs/cifar_poem_threshold_sweep.csv")
...
def load_model(model_ckpt: str | Path, device: torch.device) -> torch.nn.Module:
    if MNISTCNN is None:
        raise ImportError("MNIST dependencies are unavailable. Use --mode cifar_saved_windows or install the MNIST experiment package.")
...
def load_source_holdout_windows(config: PoemConfig) -> List[Subset]:
    if get_clean_mnist_datasets is None:
        raise ImportError("MNIST data helpers are unavailable. Use --mode cifar_saved_windows.")
...
def load_clean_window_entropies_from_csv(summary_csv_path: Path) -> np.ndarray:
    with summary_csv_path.open("r", encoding="utf-8", newline="") as handle:
        rows = list(csv.DictReader(handle))
    if not rows:
        raise ValueError(f"No clean calibration windows found in {summary_csv_path}.")
    return np.asarray([float(row["window_entropy"]) for row in rows], dtype=np.float64)


def attach_oracle_labels_to_step5_rows(rows: List[dict]) -> List[dict]:
    """
    Match the existing experiment oracle used for full Step 5 window comparisons.

    Failure pools stay positive, benign pools stay negative, and unassigned windows
    are labeled using the global mean window accuracy only for evaluation.
    """
...
def load_step5_rows_for_cifar(window_csv_path: Path) -> List[dict]:
    ...
    return attach_oracle_labels_to_step5_rows(rows)
...
def run_poem_on_cifar_saved_windows(config: PoemConfig) -> None:
    clean_csv_path = PROJECT_ROOT / config.cifar_clean_window_csv
    step5_csv_path = PROJECT_ROOT / config.cifar_step5_window_csv
    output_per_window_csv = PROJECT_ROOT / config.cifar_output_per_window_csv
    output_per_corruption_csv = PROJECT_ROOT / config.cifar_output_per_corruption_csv
    output_per_corruption_json = PROJECT_ROOT / config.cifar_output_per_corruption_json
    output_threshold_sweep_csv = PROJECT_ROOT / config.cifar_output_threshold_sweep_csv

    output_per_window_csv.parent.mkdir(parents=True, exist_ok=True)
    output_per_corruption_csv.parent.mkdir(parents=True, exist_ok=True)

    source_window_entropies = load_clean_window_entropies_from_csv(clean_csv_path)
    Fs = EmpiricalCDF(source_window_entropies)
    step5_rows = [row for row in load_step5_rows_for_cifar(step5_csv_path) if str(row["corruption_type"]) != "clean"]
    ...
    threshold_grid = [0.1, 0.3, 1.0, 3.0, 10.0, 30.0, 100.0, 300.0, 1000.0]

    for corruption_type, rows in sorted(grouped_rows.items()):
        wealth = 1.0
        epsilon = 0.0
        gradients: List[float] = []
        traced_rows: List[dict] = []

        ordered_rows = sorted(rows, key=lambda row: int(float(row["window_index"])))
        for step_index, row in enumerate(ordered_rows):
            mean_entropy = float(row["window_entropy"])
            u = Fs(mean_entropy)
            ...
            trigger = int(wealth_trigger and step_index >= config.action_delay_steps)
            traced_row = {
                ...
                "u": float(u),
                "wealth": wealth,
                "log10_wealth": float(np.log10(max(wealth, 1e-12))),
                "epsilon": epsilon,
            ...
            traced_rows.append(traced_row)
            per_window_rows.append(traced_row)
            epsilon = epsilon_next

        metrics = compute_binary_metrics(traced_rows)
        wealth_values = np.asarray([float(row["wealth"]) for row in traced_rows], dtype=np.float64)
        mean_wealth = float(np.mean(wealth_values))
        first_cross = next(
            (int(row["window_index"]) for row in traced_rows if float(row["wealth"]) >= config.wealth_threshold),
            -1,
        )
        per_corruption_rows.append(
            {
                ...
                "log10_mean_wealth": float(np.log10(max(mean_wealth, 1e-12))),
                "max_wealth": float(np.max(wealth_values)),
                "first_cross_ge_threshold": first_cross,
                "wealth_threshold": config.wealth_threshold,
        ...
        for threshold in threshold_grid:
            threshold_rows = []
            for step_index, row in enumerate(traced_rows):
                updated = dict(row)
                updated["trigger"] = int(float(row["wealth"]) >= threshold and step_index >= config.action_delay_steps)
                threshold_rows.append(updated)
            threshold_metrics = compute_binary_metrics(threshold_rows)
            sweep_rows.append(
                {
                    "corruption_type": corruption_type,
                    "wealth_threshold": threshold,
                    ...
                    "oracle_positive_windows": int(sum(int(row["oracle_trigger"]) for row in threshold_rows)),
                }
            )

    with output_per_window_csv.open("w", newline="", encoding="utf-8") as handle:
        writer = csv.DictWriter(handle, fieldnames=list(per_window_rows[0].keys()))
        writer.writeheader()
        writer.writerows(per_window_rows)

    with output_per_corruption_csv.open("w", newline="", encoding="utf-8") as handle:
        writer = csv.DictWriter(handle, fieldnames=list(per_corruption_rows[0].keys()))
        writer.writeheader()
        writer.writerows(per_corruption_rows)

    with output_threshold_sweep_csv.open("w", newline="", encoding="utf-8") as handle:
    ...
    payload = {
        "mode": config.mode,
        "clean_window_csv": str(clean_csv_path),
        ...
        "per_window_csv": str(output_per_window_csv),
        "threshold_sweep_csv": str(output_threshold_sweep_csv),
    }
    output_per_corruption_json.write_text(json.dumps(payload, indent=2), encoding="utf-8")
    (PROJECT_ROOT / config.output_threshold_json).write_text(
        json.dumps(
            {
                "wealth_threshold": config.wealth_threshold,
                "martingale_clip": config.martingale_clip,
                ...
                "action_delay_steps": config.action_delay_steps,
                "trigger_logic": "wealth_only_after_delay",
                "score_for_poem": "wealth",
                "note": "Accuracy is reported for evaluation only and is not part of the POEM trigger.",
            },
            indent=2,
        ),
        encoding="utf-8",
...
def main() -> None:
    ...
    config = PoemConfig(
        ...
        cifar_clean_window_csv=args.cifar_clean_window_csv,
        ...
        cifar_output_threshold_sweep_csv=args.cifar_output_threshold_sweep_csv,
    )
    ...
    if config.mode == "cifar_saved_windows":
        logging.info("Running POEM on saved CIFAR-10 / CIFAR-10-C windows for fair trigger comparison")
        run_poem_on_cifar_saved_windows(config)
        logging.info("Saved CIFAR POEM outputs to %s", PROJECT_ROOT / config.cifar_output_per_corruption_csv)
    else:
        logging.info("Using mixed window stream to match the existing MNIST experiment")
        if set_seed is None:
