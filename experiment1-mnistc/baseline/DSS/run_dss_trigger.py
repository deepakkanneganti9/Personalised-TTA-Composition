...
import composition_aware_fl_tta_mnist as exp
...
class FeatureOnlyCNN(nn.Module):
    ...
    def __init__(self, model: exp.SimpleMNISTCNN) -> None:
        super().__init__()
        self.features = model.features
        self.hidden_linear = model.classifier[1]
        self.hidden_activation = model.classifier[2]
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "on the existing Experiment 1 Step 5/6 artifacts."
        )
    )
    parser.add_argument(
        "--experiment-output-dir",
        type=str,
        default="/Users/deepakkanneganti/Documents/Experiment 1/outputs_experiment_w64",
    )
    parser.add_argument(
        "--checkpoint-path",
        type=str,
        default="/Users/deepakkanneganti/Documents/Experiment 1/outputs_experiment_w64/checkpoints/mls_composition.pt",
    )
    parser.add_argument(
        "--mnist-c-root",
        type=str,
    ...
    parser.add_argument(
        "--output-dir",
        type=str,
        default="/Users/deepakkanneganti/Documents/Experiment 1/baseline/DSS/outputs",
    )
...
def load_source_feature_extractor(checkpoint_path: Path, device: torch.device) -> nn.Module:
    model = exp.SimpleMNISTCNN().to(device)
...
def load_corruption_dataset(
    ...
    dataset, _ = exp.load_mnist_c_dataset(
...
def main() -> None:
    ...
    for corruption_name, corruption_rows in sorted(grouped_step5_rows.items()):
        dataset = load_corruption_dataset(
            mnist_c_root=Path(config.mnist_c_root),
            corruption_name=corruption_name,
            split=config.split,
