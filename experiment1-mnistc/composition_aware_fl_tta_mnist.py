#!/usr/bin/env python3
"""
Composition-aware trigger mechanism for federated-learning TTA on MNIST.

This file currently implements Step 1 from the requested experiment:
...
class FederatedConfig:
    """Step 2 configuration for clean-MNIST federated training."""
...
class SimpleMNISTCNN(nn.Module):
    """Simple CNN suitable for MNIST and lightweight FL experiments."""
...
def compute_input_features(images: torch.Tensor, threshold: float = 0.1) -> torch.Tensor:
    ...
    Accepts shapes [N, 1, H, W] or [N, H, W].
    """
...
def load_clean_mnist_datasets(data_root: Path, val_split: int) -> Tuple[Dataset, Dataset, Dataset]:
    """
    Step 2 - load clean MNIST from the existing local dataset root.
    """
...
def assign_digit_biased_clients(targets: Sequence[int], seed: int) -> List[List[int]]:
    """
    Create a mild-to-moderate non-IID split:
      client 1: mostly 0,1,2,3
      client 2: mostly 3,4,5,6
      client 3: mostly 6,7,8,9

    The split is overlapping in label support, but each sample is assigned to one client.
    """
    ...
    for client_id, digits in dominant_digits.items():
        for digit in digits:
            weights[client_id, digit] = 0.70
...
class NumpyImageDataset(Dataset):
    """Dataset wrapper for local MNIST-C numpy arrays."""
...
def run_step2_federated_training(
    ...
    """
    Step 2 - train the federated model on clean MNIST using FedAvg.
    """
    train_dataset, val_dataset, test_dataset = load_clean_mnist_datasets(
        data_root=Path(federated_config.data_root),
        val_split=federated_config.val_split,
    )
    train_targets = subset_targets(train_dataset)
    client_indices = assign_digit_biased_clients(train_targets, seed=seed)
    ...
    global_model = SimpleMNISTCNN().to(device)
    ...
    for round_idx in range(1, federated_config.rounds + 1):
        ...
        for client_idx, loader in enumerate(client_loaders, start=1):
            client_model = SimpleMNISTCNN().to(device)
            ...
            client_metrics[f"client_{client_idx}"] = metrics
            print(
                f"  client {client_idx}: "
    ...
    save_checkpoint(
        ...
        output_dirs["checkpoints"] / "mls_composition.pt",
    )
    return final_summary


def load_global_model_checkpoint(checkpoint_path: Path, device: torch.device) -> SimpleMNISTCNN:
...
def collect_windowed_clean_statistics(
    ...
    max_probabilities = probabilities.max(dim=1).values
    entropies = compute_sample_entropy(probabilities, epsilon)
    correctness = (predicted_labels == labels_tensor).to(torch.int64)
    input_features = compute_input_features(images_tensor)
    ...
    for start in range(0, total_seen, window_size):
        ...
        window_logits = logits_tensor[start:end]
        window_images = images_tensor[start:end]
        window_labels = labels_tensor[start:end]
        ...
        window_accuracy = float(window_correctness.float().mean().item())
        row = {
            "window_index": window_index,
            "start_index": start,
            ...
            "matched_context_placeholder": "clean_mnist",
        }
    ...
    arrays = {
        ...
        "max_probabilities": max_probabilities.numpy(),
        "correctness_flags": correctness.numpy(),
...
def resolve_mnist_c_split_paths(mnist_c_root: Path, corruption_name: str, split: str) -> Tuple[Path, Path]:
    corruption_dir = mnist_c_root / corruption_name
    images_path = corruption_dir / f"{split}_images.npy"
    labels_path = corruption_dir / f"{split}_labels.npy"
    if not images_path.exists() or not labels_path.exists():
        raise FileNotFoundError(
            f"Missing MNIST-C files for corruption='{corruption_name}', split='{split}' under {corruption_dir}."
        )
    return images_path, labels_path


def load_mnist_c_dataset(
    mnist_c_root: Path,
    corruption_name: str,
    split: str,
    ...
    """
    Load one local MNIST-C corruption dataset.

    The local copy in this workspace stores one corruption per folder, with
    10,000 examples in `test` and 60,000 examples in `train`. Step 4 now uses
    the selected corruption categories from the train split by default and, if
    requested, takes the first N samples from that file.
    """
    images_path, labels_path = resolve_mnist_c_split_paths(mnist_c_root, corruption_name, split)
    images = np.load(images_path)
    labels = np.load(labels_path)

    slice_start = 0
    slice_end = int(labels.shape[0])
    if max_samples is not None:
        slice_end = min(slice_start + max_samples, int(labels.shape[0]))
        images = images[slice_start:slice_end]
        labels = labels[slice_start:slice_end]
    ...
    metadata = {
        ...
        "severity_note": (
            "Local MNIST-C train files contain 60,000 samples per corruption. "
            "This pipeline uses the first selected slice from that file rather than an inferred severity band."
        ),
    }
    return dataset, metadata


def list_available_mnist_c_corruptions(mnist_c_root: Path) -> List[str]:
    return sorted(path.name for path in mnist_c_root.iterdir() if path.is_dir())
...
def load_historical_context_bank(history_dir: Path) -> List[HistoricalContext]:
    ...
    if not bank_path.exists():
        clean_path = history_dir / "context_1_clean_mnist.json"
...
def infer_local_severity_bucket(sample_index: int, bucket_size: int) -> int:
    """
    Infer a local severity-style bucket from index position in 60k train files.
    This is saved as an inferred field, not as a verified MNIST-C severity label.
    """
...
def run_step3_clean_historical_bank(
    ...
    _, val_dataset, _ = load_clean_mnist_datasets(
        data_root=Path(federated_config.data_root),
        val_split=federated_config.val_split,
    )
    val_loader = create_eval_loader(
    ...
    save_json(asdict(clean_context), history_dir / "context_1_clean_mnist.json")
    ...
    summary = {
        "clean_context_profile_path": str(history_dir / "context_1_clean_mnist.json"),
        ...
        "coverage_checklist": {
            ...
            "historical_context_1_clean_mnist": True,
...
def run_step4_known_corruption_contexts(
    ...
    """
    print(
        "Step 4: building known corruption contexts from local MNIST-C "
    ...
    clean_context_path = history_dir / "context_1_clean_mnist.json"
    ...
    for idx, corruption_name in enumerate(corruption_context_config.context_names, start=2):
        dataset, dataset_meta = load_mnist_c_dataset(
    ...
    combined_index = {
        ...
        "selection_note": (
            "Step 4 used local per-corruption MNIST-C train files. "
            "For each selected corruption, the first requested sample slice was used "
            "(default: first 10,000 samples) instead of assuming explicit in-file severity bands."
        ),
    }
    save_json(combined_index, history_dir / "historical_context_bank.json")
    save_json(
        {
            "mnist_c_root": str(mnist_c_root),
...
def scan_corruption_dataset(
    ...
    predicted_labels = probabilities.argmax(dim=1)
    max_probabilities = probabilities.max(dim=1).values
    entropies = compute_sample_entropy(probabilities, trigger_config.epsilon)
    input_features = compute_input_features(images_tensor)
    correctness = (predicted_labels == labels_tensor).to(torch.int64)
    ...
    for local_idx in range(labels_tensor.shape[0]):
        ...
        sample_rows.append(
            {
                ...
                "true_label": int(labels_tensor[local_idx].item()),
                "predicted_label": int(predicted_labels[local_idx].item()),
                "is_correct": int(correctness[local_idx].item()),
                "entropy": float(entropies[local_idx].item()),
                "max_softmax_probability": float(max_probabilities[local_idx].item()),
    ...
    for start in range(0, labels_tensor.shape[0], window_size):
        end = min(start + window_size, labels_tensor.shape[0])
        window_index = start // window_size
        window_logits = logits_tensor[start:end]
        window_images = images_tensor[start:end]
        window_labels = labels_tensor[start:end]
        ...
        window_rows.append(
            {
                ...
                "severity_is_inferred": 1,
                "window_accuracy": float(window_correctness.float().mean().item()),
                "window_entropy": float(window_entropies.mean().item()),
                "window_max_probability": float(window_probs.mean().item()),
                "window_wrong_fraction": 1.0 - float(window_correctness.float().mean().item()),
                "matched_context": matched_context.name,
                ...
                "input_feature_mean_json": json.dumps([float(x) for x in window_input.mean(dim=0).tolist()]),
            }
        )
        for cls_idx, value in enumerate(window_stats.output_distribution):
...
def ensure_required_window_pools(
    ...
    pool_specs = {
        "stable": lambda row: (
            acc(row) >= 0.92
            and dist(row) <= np.quantile([dist(r) for r in window_rows], 0.50)
        ),
        "mild_variability": lambda row: (
            acc(row) >= 0.85
            and dist(row) <= np.quantile([dist(r) for r in window_rows], 0.75)
        ),
    ...
    score_fns = {
        ...
        "mild_variability": lambda row: (acc(row), -abs(dist(row) - 0.5 * np.quantile([dist(r) for r in window_rows], 0.75)), -ent(row)),
...
def compute_binary_metrics(rows: Sequence[Dict[str, object]], decision_key: str) -> Dict[str, float]:
    ...
    visible_mask = np.asarray(
        [1 if "visible_failure" in str(row["region_label"]).lower() else 0 for row in rows],
        dtype=np.int64,
    )
    hidden_mask = np.asarray(
        [1 if "hidden_failure" in str(row["region_label"]).lower() else 0 for row in rows],
        dtype=np.int64,
    )
...
def load_logits_for_window(
    ...
    if cache_key not in logits_cache:
        logits_path = output_dirs["pools"] / f"scan_{corruption_type}_{split}_logits.npy"
        if not logits_path.exists():
            raise FileNotFoundError(
                f"ARS baseline requires saved logits at {logits_path}. Re-run Step 5 if needed."
            )
...
def run_step5_scan_and_mine_pools(
    ...
    corruption_names = (
        ...
        else [name for name in list_available_mnist_c_corruptions(mnist_c_root) if name != "identity"]
    )
    ...
    for corruption_name in corruption_names:
        dataset, metadata = load_mnist_c_dataset(
            mnist_c_root=mnist_c_root,
            corruption_name=corruption_name,
        ...
        sample_rows, window_rows, arrays = scan_corruption_dataset(
            ...
            sample_index_offset=int(metadata["slice_start"]),
        )
    ...
    summary = {
        ...
        "severity_note": (
            "Severity values in Step 5 are inferred from index buckets within local train files "
            f"using bucket_size={pool_mining_config.severity_bucket_size}; they are not official MNIST-C labels."
        ),
    }
...
def create_demo_window(
    ...
    images = torch.rand(samples, 1, 28, 28) * 0.25
    yy, xx = torch.meshgrid(torch.arange(28), torch.arange(28), indexing="ij")
    cx = 14.0 + float(input_shift[0])
    cy = 14.0 + float(input_shift[1])
    radius = 6.0 + float(input_shift[2])
    blob = (((xx - cx) ** 2 + (yy - cy) ** 2) < radius ** 2).float()
    images = torch.clamp(images + 0.65 * blob.unsqueeze(0).unsqueeze(0), 0.0, 1.0)
...
def build_demo_contexts() -> List[HistoricalContext]:
    ...
    return [
        build_historical_context("clean_mnist", clean_windows, {"type": "clean"}),
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Composition-aware FL-TTA experiment on MNIST. Step 1 to Step 10 are implemented."
    )
    parser.add_argument("--output-dir", type=str, default="outputs_experiment", help="Directory for experiment artifacts.")
    parser.add_argument(
        "--data-root",
        type=str,
        default="/Users/deepakkanneganti/Documents/Experiment 1/Datasets/MNIST",
        help="Existing local MNIST dataset root.",
    )
    parser.add_argument(
        "--mnist-c-root",
        type=str,
        default="/Users/deepakkanneganti/Documents/Experiment 1/Datasets/MNIST/mnist_c/mnist_c",
        help="Existing local MNIST-C root with one folder per corruption.",
    )
    ...
    parser.add_argument(
        "--skip-demo",
        action="store_true",
        help="Only validate configuration and write config JSON without running the synthetic demo.",
    )
    ...
    parser.add_argument(
        ...
        help="Comma-separated MNIST-C corruption names to use as Step 4 historical contexts.",
    )
    parser.add_argument(
        "--mnist-c-split",
        type=str,
        default="train",
        choices=["train", "test"],
        help="MNIST-C split to use for Step 4. Default is train.",
    )
    parser.add_argument(
        "--max-context-samples",
        type=int,
        default=10000,
        help="Number of leading samples to use per Step 4 context. Default is the first 10,000.",
    )
    parser.add_argument(
        "--pool-corruptions",
        type=str,
        default="",
        help="Optional comma-separated corruption names for Step 5. Empty means scan all available corruptions except identity.",
    )
    parser.add_argument(
        "--pool-split",
        type=str,
        default="train",
        choices=["train", "test"],
        help="MNIST-C split to scan for Step 5.",
    )
    parser.add_argument(
        "--max-pool-samples",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=10000,
        help="Bucket size for inferred local severity labels in Step 5.",
    )
    parser.add_argument(
        "--windows-per-segment",
        type=int,
