...
FILTERED_SUB_PATH = Path(__file__).resolve().parent / "STM" / "results" / "filtered_improved_random_substitution.csv"
CATM_PATH = Path(__file__).resolve().parent / "CATM" / "results" / "filtered_same_scale.csv"
...
def main():
    ...
    for row in filtered_rows:
        ...
        length = int(row["performing_length"])
        ...
        grouped[(corruption, length)].append(
            {
                ...
                "STAM": float(row["protected_v2_mixed_accuracy"]),
    ...
    summary_rows = []
    for (corruption, length), rows in sorted(grouped.items(), key=lambda item: (item[0][0], item[0][1])):
        summary_rows.append(
            {
                "Corruption": corruption,
                "Length": length,
                ...
                "STAM": mean([r["STAM"] for r in rows]),
    ...
    csv_path = RESULTS_DIR / "final_table_by_corruption_and_length_50_50.csv"
    json_path = RESULTS_DIR / "final_table_by_corruption_and_length_50_50.json"
    md_path = RESULTS_DIR / "final_table_by_corruption_and_length_50_50.md"
