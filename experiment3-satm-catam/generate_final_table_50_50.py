...
def main():
    ...
    for row in filtered_rows:
        ...
        grouped[(tta_method, length)].append(enriched)

    summary_rows = []
    for (method, length), rows in sorted(grouped.items(), key=lambda item: (item[0][0], item[0][1])):
        summary_rows.append(
            {
                "Method": method,
                "Length": length,
    ...
    csv_path = RESULTS_DIR / "final_table_50_50.csv"
    json_path = RESULTS_DIR / "final_table_50_50.json"
    md_path = RESULTS_DIR / "final_table_50_50.md"
