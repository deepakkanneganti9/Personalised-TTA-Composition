...
DEFAULT_CONFIG = {
    ...
    "preferred_classes_per_client": 4,
...
def build_random_client_preferences(num_clients: int, preferred_classes_per_client: int, seed: int):
    ...
    while True:
        ...
        for client_id in range(num_clients):
            chosen = sorted(rng.choice(classes, size=preferred_classes_per_client, replace=False).tolist())
            preferences[f"client_{client_id}"] = chosen
            class_coverage.update(chosen)

        if min(class_coverage[class_id] for class_id in classes) >= 1:
            return preferences
...
def build_random_mild_noniid_split(labels, config):
    ...
    preferences = build_random_client_preferences(
        ...
        preferred_classes_per_client=config["preferred_classes_per_client"],
        seed=config["seed"],
    )
...
def get_run_dir(run_dir_arg: Optional[str]) -> Path:
    ...
    return (ARTIFACT_ROOT / f"cifar10_fl_run_{datetime.now().strftime('%Y%m%d_%H%M%S')}").resolve()
...
def run_round(run_dir: Path, round_index: int, device_name: str = "cpu"):
    ...
    print(f"round={round_index} accuracy={accuracy:.4f} device={device}", flush=True)
