...
class ASRConfig:
    ...
    alpha0_grid: Sequence[float]
    mu_c_grid: Sequence[float]
    cifar_output_per_corruption_best_csv: str
    cifar_output_sweep_csv: str
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--alpha0-grid", default="0.1,0.2,0.3,0.5,0.7,0.9")
    parser.add_argument("--mu-c-grid", default="0.90,0.95,0.97,0.99")
    parser.add_argument("--cifar_output_per_corruption_best_csv", default="ARS/outputs/cifar_asr_per_corruption_best.csv")
    parser.add_argument("--cifar_output_sweep_csv", default="ARS/outputs/cifar_asr_sweep.csv")
...
def compute_c_t_from_logits(window_logits: np.ndarray) -> float:
    if window_logits.ndim != 2 or window_logits.shape[1] != 10:
        raise ValueError(f"Expected logits [N,10], got {window_logits.shape}")
...
def run_asr_trace_for_rows(
    ...
    *,
    ...
    num_classes = 10
    c_bar_init = float(-math.log(alpha0 * num_classes))
    ...
    for row in rows:
        ...
        if trigger and reset_on_trigger:
            c_bar_new = c_bar_init
        else:
            c_bar_new = float(mu_c * c_bar_prev + (1.0 - mu_c) * c_t)
        traced = {
            "corruption_type": str(row["corruption_type"]),
...
def run_asr_on_cifar_saved_windows(config: ASRConfig) -> None:
    ...
    per_corruption_best_csv = PROJECT_ROOT / config.cifar_output_per_corruption_best_csv
    sweep_csv = PROJECT_ROOT / config.cifar_output_sweep_csv
    ...
    per_corruption_best_rows: List[dict] = []
    ...
    for corruption_type, corruption_rows in sorted(grouped_rows.items()):
        ...
        traced_rows, c_bar_init = run_asr_trace_for_rows(
            rows=ordered_rows,
        ...
        per_window_rows.extend(traced_rows)
        ...
        per_corruption_rows.append(default_row)

        best_row = None
        best_score = None
        for alpha0 in config.alpha0_grid:
            for mu_c in config.mu_c_grid:
                candidate_rows, candidate_c_bar_init = run_asr_trace_for_rows(
                    rows=ordered_rows,
                    logits_cache=logits_cache,
                    pools_dir=pools_dir,
                    alpha0=alpha0,
                ...
                candidate_metrics = compute_binary_metrics(candidate_rows)
                sweep_row = {
                    "corruption_type": corruption_type,
                    "alpha0": alpha0,
                    "mu_c": mu_c,
                    "mean_accuracy": float(np.mean([float(row["window_accuracy"]) for row in candidate_rows])),
                    "mean_entropy": float(np.mean([float(row["mean_entropy"]) for row in candidate_rows])),
                    "mean_C_t": float(np.mean([float(row["C_t"]) for row in candidate_rows])),
                    "mean_C_bar_prev": float(np.mean([float(row["C_bar_prev"]) for row in candidate_rows])),
                    "initial_C_bar": candidate_c_bar_init,
                    "oracle_positive_windows": int(sum(int(row["oracle_trigger"]) for row in candidate_rows)),
                    ...
                    "tn": candidate_metrics["tn"],
                    "fp": candidate_metrics["fp"],
                    "fn": candidate_metrics["fn"],
                }
                sweep_rows.append(sweep_row)
                score = (sweep_row["f1"], sweep_row["accuracy"], sweep_row["recall"], sweep_row["precision"])
    ...
    with per_corruption_best_csv.open("w", newline="", encoding="utf-8") as handle:
        writer = csv.DictWriter(handle, fieldnames=list(per_corruption_best_rows[0].keys()))
        writer.writeheader()
        writer.writerows(per_corruption_best_rows)
        ...
        json.dumps(
            {
                ...
                "per_corruption_best_rows": per_corruption_best_rows,
                "per_window_csv": str(per_window_csv),
                "per_corruption_best_csv": str(per_corruption_best_csv),
        ...
        json.dumps(
            {
                "mu_c": config.mu_c,
                "alpha0": config.alpha0,
                "mu_c_grid": list(config.mu_c_grid),
                "alpha0_grid": list(config.alpha0_grid),
                "reset_on_trigger": config.reset_on_trigger,
                "window_size": config.window_size,
                "trigger_logic": "trigger if C_t > C_bar_prev; reset C_bar to -log(alpha0 * C) on trigger",
...
def main() -> None:
    ...
    config = ASRConfig(
        ...
        alpha0_grid=tuple(parse_float_list(args.alpha0_grid)),
        mu_c_grid=tuple(parse_float_list(args.mu_c_grid)),
        cifar_output_per_corruption_best_csv=args.cifar_output_per_corruption_best_csv,
        cifar_output_sweep_csv=args.cifar_output_sweep_csv,
    )
