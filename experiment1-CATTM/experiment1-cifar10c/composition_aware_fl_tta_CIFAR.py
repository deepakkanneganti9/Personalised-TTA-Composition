#!/usr/bin/env python3
"""
Composition-aware trigger mechanism for federated-learning TTA on CIFAR-10.

This file currently implements the CIFAR-10 variants of Step 1 and Step 2:
...
class FederatedConfig:
    """Step 2 configuration for clean-CIFAR-10 federated training."""
...
class SimpleCIFARCNN(nn.Module):
    """Simple CNN suitable for CIFAR-10 and lightweight FL experiments."""
...
def compute_output_distribution(predicted_labels: torch.Tensor, num_classes: int = 10) -> torch.Tensor:
    """
...
def compute_input_features(images: torch.Tensor, threshold: float = 0.1) -> torch.Tensor:
    ...
    if images.ndim == 4 and images.shape[1] == 3:
        ...
        if float(rgb.min().item()) < 0.0 or float(rgb.max().item()) > 1.0:
            mean = torch.tensor([0.4914, 0.4822, 0.4465], device=rgb.device, dtype=rgb.dtype).view(1, 3, 1, 1)
            std = torch.tensor([0.2470, 0.2435, 0.2616], device=rgb.device, dtype=rgb.dtype).view(1, 3, 1, 1)
...
def compute_window_stats(
    ...
    output_distribution = compute_output_distribution(predicted_labels)
...
def compute_window_stats_from_numpy(
    ...
    output_distribution = np.bincount(predicted_labels, minlength=10).astype(np.float64)
...
def load_clean_cifar10_datasets(data_root: Path, val_split: int) -> Tuple[Dataset, Dataset, Dataset]:
    """
    Step 2 - load clean CIFAR-10 from the existing local dataset root.
    """
    transform = make_cifar_transform()
    train_dataset = datasets.CIFAR10(root=str(data_root), train=True, download=False, transform=transform)
...
def assign_class_biased_clients(targets: Sequence[int], seed: int) -> List[List[int]]:
    ...
    weights = np.full((3, 10), 0.10, dtype=np.float64)
...
def summarize_client_split(targets: Sequence[int], client_indices: Sequence[Sequence[int]]) -> Dict[str, Dict]:
    ...
    for client_id, indices in enumerate(client_indices, start=1):
        ...
        label_hist = np.bincount(labels, minlength=10).tolist() if labels else [0] * 10
...
class NumpyImageDataset(Dataset):
    ...
    def __getitem__(self, index: int) -> Tuple[torch.Tensor, int]:
        ...
        if image_tensor.ndim == 3 and image_tensor.shape[0] == 3:
            mean = torch.tensor([0.4914, 0.4822, 0.4465], dtype=image_tensor.dtype).view(3, 1, 1)
            std = torch.tensor([0.2470, 0.2435, 0.2616], dtype=image_tensor.dtype).view(3, 1, 1)
...
def run_step2_federated_training(
    ...
    """
    Step 2 - train the federated model on clean CIFAR-10 using FedAvg.
    """
    ...
    train_dataset, val_dataset, test_dataset = load_clean_cifar10_datasets(
...
def collect_windowed_clean_statistics(
    ...
    for start in range(0, total_seen, window_size):
        ...
        row = {
            ...
            "matched_context_placeholder": "clean_cifar10",
        }
...
def resolve_cifar10_c_paths(cifar10_c_root: Path, corruption_name: str) -> Tuple[Path, Path]:
    ...
    if not images_path.exists() or not labels_path.exists():
        raise FileNotFoundError(
            f"Missing CIFAR-10-C files for corruption='{corruption_name}' under {cifar10_c_root}."
        )
...
def load_cifar10_c_dataset(
    ...
    """
    Load one local CIFAR-10-C corruption dataset using official severity blocks.
    """
    if split != "test":
        raise ValueError("Local CIFAR-10-C only provides test corruption files, so split must be 'test'.")
    ...
    for severity in selected_severities:
        if severity < 1 or severity > 5:
            raise ValueError("CIFAR-10-C severities must be in {1,2,3,4,5}.")
    ...
    metadata = {
        ...
        "severity_note": "CIFAR-10-C uses official 10,000-sample blocks for severities 1..5.",
    }
...
def load_historical_context_bank(history_dir: Path) -> List[HistoricalContext]:
    ...
    if not bank_path.exists():
        clean_path = history_dir / "context_1_clean_cifar10.json"
...
def infer_local_severity_bucket(sample_index: int, bucket_size: int) -> int:
    """
    Infer the CIFAR-10-C severity bucket from the 10,000-sample block index.
    """
...
def run_step3_clean_historical_bank(
    ...
    _, val_dataset, _ = load_clean_cifar10_datasets(
        data_root=Path(federated_config.data_root),
        val_split=federated_config.val_split,
    )
    val_loader = create_eval_loader(
    ...
    save_json(asdict(clean_context), history_dir / "context_1_clean_cifar10.json")
    ...
    summary = {
        "clean_context_profile_path": str(history_dir / "context_1_clean_cifar10.json"),
        ...
        "coverage_checklist": {
            ...
            "historical_context_1_clean_cifar10": True,
...
def run_step4_known_corruption_contexts(
    ...
    """
    print(
        "Step 4: building known corruption contexts from local CIFAR-10-C "
    ...
    clean_context_path = history_dir / "context_1_clean_cifar10.json"
    ...
    combined_index = {
        ...
        "selection_note": (
            "Step 4 used local per-corruption CIFAR-10-C files and the official severity-1 block "
...
def step5_window_row_to_window_stats(row: Dict[str, object]) -> WindowStats:
    ...
    output_distribution = [float(row.get(f"output_dist_{cls_idx}", 0.0)) for cls_idx in range(10)]
...
def compute_asr_c_t_from_logits(window_logits: np.ndarray) -> float:
    ...
    if window_logits.ndim != 2 or window_logits.shape[1] != 10:
        raise ValueError(f"Expected window logits with shape [N, 10], got {window_logits.shape}.")
...
def run_step9_evaluate_methods(
    ...
    """
    Step 9 - evaluate proposed trigger against baselines and save plots.
    """
    print("Step 9: evaluating baselines and proposed trigger.")
    ...
    clean_window_entropies = load_clean_window_entropies(output_dirs)

    methods = {
        "entropy_only": lambda rows: baseline_entropy_only(rows, evaluation_config.entropy_threshold),
        "proxy_only": lambda rows: baseline_proxy_only(rows, trigger_config.trigger_threshold),
        "input_only": lambda rows: baseline_input_only(rows, trigger_config.trigger_threshold),
        "proposed": lambda rows: [int(bool(row["trigger"])) for row in rows],
    }
    ...
    for stream_name, meta in step8_summary["step8_stream_summaries"].items():
        ...
        poem_rows = run_poem_on_rows(
            rows=rows,
            clean_window_entropies=clean_window_entropies,
            evaluation_config=evaluation_config,
            window_size=historical_bank_config.window_size,
        )
        asr_rows = run_asr_on_rows(
            rows=rows,
        ...
        save_json(
            {
                "stream_name": stream_name,
                "poem_config": {
                    "wealth_threshold": evaluation_config.poem_wealth_threshold,
                    "martingale_clip": evaluation_config.poem_martingale_clip,
                    "martingale_gamma": evaluation_config.poem_martingale_gamma,
                    "action_delay_samples": evaluation_config.poem_action_delay_samples,
                    "action_delay_steps": max(
                        1,
                        int(math.ceil(evaluation_config.poem_action_delay_samples / max(historical_bank_config.window_size, 1))),
                    ),
                },
                "results": poem_rows,
            },
            output_dirs["window_results"] / f"{stream_name}_poem_results.json",
        )
        save_csv_rows(poem_rows, output_dirs["window_results"] / f"{stream_name}_poem_results.csv")
        save_json(
            {
                "stream_name": stream_name,
                "asr_config": {
                    "mu_c": evaluation_config.asr_mu_c,
                    "alpha0": evaluation_config.asr_alpha0,
                    "reset_on_trigger": evaluation_config.asr_reset_on_trigger,
                    "c_bar_init": float(-math.log(evaluation_config.asr_alpha0 * 10)),
                },
                "results": asr_rows,
            },
            output_dirs["window_results"] / f"{stream_name}_asr_results.json",
        )
        save_csv_rows(asr_rows, output_dirs["window_results"] / f"{stream_name}_asr_results.csv")

        stream_metrics: Dict[str, object] = {}
        for method_name, decision_fn in methods.items():
            decisions = decision_fn(rows)
            decision_key = f"{method_name}_decision"
            method_rows = attach_decisions(rows, decision_key, decisions)
            metrics = compute_binary_metrics(method_rows, decision_key)
            stream_metrics[method_name] = metrics

            metric_row = {"stream_name": stream_name, "method": method_name}
            metric_row.update(metrics)
            metric_row["accuracy"] = safe_divide(
                metrics["tp"] + metrics["tn"],
                metrics["tp"] + metrics["tn"] + metrics["fp"] + metrics["fn"],
            )
            all_metric_rows.append(metric_row)

        poem_metrics = compute_binary_metrics(poem_rows, "poem_decision")
        stream_metrics["poem"] = poem_metrics
        poem_metric_row = {"stream_name": stream_name, "method": "poem"}
        poem_metric_row.update(poem_metrics)
        poem_metric_row["accuracy"] = safe_divide(
            poem_metrics["tp"] + poem_metrics["tn"],
            poem_metrics["tp"] + poem_metrics["tn"] + poem_metrics["fp"] + poem_metrics["fn"],
        )
        all_metric_rows.append(poem_metric_row)

        asr_metrics = compute_binary_metrics(asr_rows, "asr_decision")
        stream_metrics["asr"] = asr_metrics
        asr_metric_row = {"stream_name": stream_name, "method": "asr"}
        asr_metric_row.update(asr_metrics)
        asr_metric_row["accuracy"] = safe_divide(
            asr_metrics["tp"] + asr_metrics["tn"],
            asr_metrics["tp"] + asr_metrics["tn"] + asr_metrics["fp"] + asr_metrics["fn"],
        )
        all_metric_rows.append(asr_metric_row)

        sweep_rows = run_joint_threshold_sweep(
            rows=rows,
            evaluation_config=evaluation_config,
        )
        for sweep_row in sweep_rows:
            tagged = {"stream_name": stream_name}
            tagged.update(sweep_row)
            all_sweep_rows.append(tagged)
        save_csv_rows(
            [{"stream_name": stream_name, **row} for row in sweep_rows],
            output_dirs["metrics"] / f"{stream_name}_joint_threshold_sweep.csv",
        )
        save_json(
            {
                "stream_name": stream_name,
            ...
            output_dirs["metrics"] / f"{stream_name}_joint_threshold_sweep.json",
        )
        ...
        stream_metrics_summary[stream_name] = {
            "methods": stream_metrics,
            "joint_threshold_sweep_csv_path": str(output_dirs["metrics"] / f"{stream_name}_joint_threshold_sweep.csv"),
            "joint_threshold_sweep_json_path": str(output_dirs["metrics"] / f"{stream_name}_joint_threshold_sweep.json"),
            "poem_results_json_path": str(output_dirs["window_results"] / f"{stream_name}_poem_results.json"),
            "poem_results_csv_path": str(output_dirs["window_results"] / f"{stream_name}_poem_results.csv"),
            "asr_results_json_path": str(output_dirs["window_results"] / f"{stream_name}_asr_results.json"),
            "asr_results_csv_path": str(output_dirs["window_results"] / f"{stream_name}_asr_results.csv"),
        }

    save_csv_rows(all_metric_rows, output_dirs["metrics"] / "step9_method_metrics.csv")
    save_csv_rows(all_sweep_rows, output_dirs["metrics"] / "step9_joint_threshold_sweep.csv")

    full_window_rows = run_trigger_on_full_step5_windows(
        output_dirs=output_dirs,
        trigger_config=trigger_config,
    )
    mean_accuracy_threshold = float(np.mean([float(row["window_accuracy"]) for row in full_window_rows]))
    labeled_full_window_rows = attach_full_window_oracle_labels(
        rows=full_window_rows,
        mean_accuracy_threshold=mean_accuracy_threshold,
    )
    save_json(
        {
            "rows": labeled_full_window_rows,
            "mean_accuracy_threshold": mean_accuracy_threshold,
        },
        output_dirs["window_results"] / "step9_full_step5_trigger_rows.json",
    )
    save_csv_rows(
        labeled_full_window_rows,
        output_dirs["window_results"] / "step9_full_step5_trigger_rows.csv",
    )

    full_window_threshold_summary: Dict[str, object] = {
        "mean_accuracy_threshold": mean_accuracy_threshold,
    }
    full_window_mean_metric_rows: List[Dict[str, object]] = []
    full_window_calibrated_metric_rows: List[Dict[str, object]] = []
    full_window_sweep_rows: List[Dict[str, object]] = []

    mean_accuracy_rows, mean_accuracy_metrics = evaluate_single_threshold_method(
        rows=labeled_full_window_rows,
        score_key="window_accuracy",
        threshold=mean_accuracy_threshold,
        greater_is_positive=False,
        decision_key="accuracy_mean_threshold_decision",
    )
    full_window_mean_metric_rows.extend(
        compute_corruption_wise_metrics(
            rows=mean_accuracy_rows,
            decision_key="accuracy_mean_threshold_decision",
            method_name="accuracy_mean_threshold",
            threshold_value=mean_accuracy_threshold,
            threshold_source="global_mean_accuracy",
        )
    )
    full_window_threshold_summary["accuracy_mean_threshold"] = {
        "threshold": mean_accuracy_threshold,
        "global_metrics": mean_accuracy_metrics,
    }

    score_methods = {
        "entropy_only": ("avg_entropy", True),
        "proxy_only": ("proxy_statistic", True),
        "input_only": ("input_statistic", True),
        "proposed": ("final_score", True),
    }

    for method_name, (score_key, greater_is_positive) in score_methods.items():
        mean_threshold = float(np.mean([float(row[score_key]) for row in labeled_full_window_rows]))
        mean_rows, mean_metrics = evaluate_single_threshold_method(
            rows=labeled_full_window_rows,
            score_key=score_key,
            threshold=mean_threshold,
            greater_is_positive=greater_is_positive,
            decision_key=f"{method_name}_mean_decision",
        )
        full_window_mean_metric_rows.extend(
            compute_corruption_wise_metrics(
                rows=mean_rows,
                ...
                threshold_value=mean_threshold,
                threshold_source="global_mean_score",
            )
        )

        calibrated_threshold, calibrated_metrics, calibrated_rows = sweep_best_threshold(
            rows=labeled_full_window_rows,
            score_key=score_key,
            greater_is_positive=greater_is_positive,
        )
        calibrated_decision_key = f"{score_key}_decision"
        full_window_calibrated_metric_rows.extend(
            compute_corruption_wise_metrics(
                rows=calibrated_rows,
                decision_key=calibrated_decision_key,
                method_name=method_name,
                threshold_value=calibrated_threshold,
                threshold_source="global_best_f1_threshold",
            )
        )

        for threshold in make_threshold_grid(np.asarray([float(row[score_key]) for row in labeled_full_window_rows], dtype=np.float64)):
            sweep_rows, sweep_metrics = evaluate_single_threshold_method(
                rows=labeled_full_window_rows,
                score_key=score_key,
                threshold=threshold,
                greater_is_positive=greater_is_positive,
                decision_key=f"{method_name}_sweep_decision",
            )
            del sweep_rows
            full_window_sweep_rows.append(
                {
                    "method": method_name,
                    "score_key": score_key,
                    "threshold": threshold,
                    ...
                    "false_negative_rate": sweep_metrics["false_negative_rate"],
                    "hidden_failure_recall": sweep_metrics["hidden_failure_recall"],
                    "visible_failure_recall": sweep_metrics["visible_failure_recall"],
                }
            )

        full_window_threshold_summary[method_name] = {
            "mean_threshold": mean_threshold,
            "mean_threshold_metrics": mean_metrics,
            "best_f1_threshold": calibrated_threshold,
            "best_f1_metrics": calibrated_metrics,
        }

    mean_final_score_threshold = float(np.mean([float(row["final_score"]) for row in labeled_full_window_rows]))
    joint_mean_decisions = baseline_accuracy_and_final_score(
        rows=labeled_full_window_rows,
        accuracy_threshold=mean_accuracy_threshold,
        final_score_threshold=mean_final_score_threshold,
    )
    joint_mean_rows = attach_decisions(
        labeled_full_window_rows,
        "joint_mean_accuracy_final_decision",
        joint_mean_decisions,
    )
    full_window_mean_metric_rows.extend(
        compute_corruption_wise_metrics(
            rows=joint_mean_rows,
            decision_key="joint_mean_accuracy_final_decision",
            method_name="joint_accuracy_plus_proposed",
            threshold_value=mean_final_score_threshold,
            threshold_source="global_mean_accuracy_and_global_mean_final_score",
        )
    )
    full_window_threshold_summary["joint_accuracy_plus_proposed"] = {
        "accuracy_threshold": mean_accuracy_threshold,
        "final_score_threshold": mean_final_score_threshold,
        "metrics": compute_binary_metrics(joint_mean_rows, "joint_mean_accuracy_final_decision"),
    }

    save_csv_rows(
        full_window_mean_metric_rows,
        output_dirs["metrics"] / "step9_full_window_corruption_metrics_mean_thresholds.csv",
    )
    save_json(
        {
            "rows": full_window_mean_metric_rows,
            "threshold_summary": full_window_threshold_summary,
        },
        output_dirs["metrics"] / "step9_full_window_corruption_metrics_mean_thresholds.json",
    )
    save_csv_rows(
        full_window_calibrated_metric_rows,
        output_dirs["metrics"] / "step9_full_window_corruption_metrics_best_f1.csv",
    )
    save_json(
        {
            "rows": full_window_calibrated_metric_rows,
            "threshold_summary": full_window_threshold_summary,
        },
        output_dirs["metrics"] / "step9_full_window_corruption_metrics_best_f1.json",
    )
    save_csv_rows(
        full_window_sweep_rows,
        output_dirs["metrics"] / "step9_full_window_threshold_sweep.csv",
    )
    ...
    summary = {
        ...
        "joint_threshold_sweep_csv_path": str(output_dirs["metrics"] / "step9_joint_threshold_sweep.csv"),
        "plots_dir": str(output_dirs["plots"]),
        "full_window_mean_threshold_metrics_csv_path": str(
            output_dirs["metrics"] / "step9_full_window_corruption_metrics_mean_thresholds.csv"
        ),
        "full_window_best_f1_metrics_csv_path": str(
            output_dirs["metrics"] / "step9_full_window_corruption_metrics_best_f1.csv"
        ),
        "full_window_threshold_sweep_csv_path": str(
            output_dirs["metrics"] / "step9_full_window_threshold_sweep.csv"
        ),
        "full_window_threshold_summary": full_window_threshold_summary,
    }
...
def run_step5_scan_and_mine_pools(
    ...
    summary = {
        ...
        "severity_note": (
            "Severity values in Step 5 follow the official CIFAR-10-C severity blocks "
...
def create_demo_window(
    ...
    """
    logits = torch.randn(samples, 10) / max(entropy_scale, 1e-4)
    logits[:, label_bias % 10] += 2.5
...
def build_demo_contexts() -> List[HistoricalContext]:
    ...
    return [
        build_historical_context("clean_cifar10", clean_windows, {"type": "clean"}),
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Composition-aware FL-TTA experiment on CIFAR-10. Step 1 and Step 2 are CIFAR-ready."
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="Dataset",
        help="Existing local CIFAR-10 dataset root.",
    )
    parser.add_argument(
        "--mnist-c-root",
        type=str,
        default="Dataset/CIFAR-10-C/CIFAR-10-C",
        help="Existing local CIFAR-10-C root with one .npy file per corruption.",
    )
    ...
    parser.add_argument(
        ...
        help="Comma-separated CIFAR-10-C corruption names to use as Step 4 historical contexts.",
    )
    parser.add_argument(
        "--mnist-c-split",
        type=str,
        default="test",
        choices=["test"],
        help="CIFAR-10-C split to use for Step 4.",
    )
    parser.add_argument(
        "--max-context-samples",
        type=int,
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="Optional comma-separated corruption names for Step 5. Empty means scan all available CIFAR-10-C corruptions.",
    )
    parser.add_argument(
        "--pool-split",
        type=str,
        default="test",
        choices=["test"],
        help="CIFAR-10-C split to scan for Step 5.",
    )
    parser.add_argument(
        "--max-pool-samples",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=10000,
        help="Severity block size for CIFAR-10-C labels in Step 5.",
    )
    parser.add_argument(
        "--windows-per-segment",
        type=int,
